...
- 首次访问默认进入登录页 `/login`
- 支持注册、登录、退出登录和会话校验
- 用户手记、月度总结、年度总结按登录用户隔离，手记图片也需要在登录态下按归属读取
...
- 登录 `/login`
  支持注册和登录，也保留演示体验账号 `demo / Yuexing@2026`；首次进入会自带示例衣柜图片、偏好和历史搭配
- 今日行程 `/app`
- 我的衣柜 `/wardrobe`
- 我的手记 `/journal`
- 个性画像 `/profile`
...
- 手记数据按当前登录用户隔离，图片资源不会再以公开静态链接方式暴露
...
- 手记图片改为落盘到服务端 `/uploads/*`，并在登录态下按用户归属读取；当前方案适合原型和演示，不适合大规模生产
- 还未接入邮件验证、短信验证和密码找回
...
- `CORS_ORIGINS` 仅用于放开允许来源；当前登录态基于同域 `HttpOnly Cookie`，如果做跨域 SPA，还需要额外调整 Cookie 策略与前端请求配置。
